...
import simulator.components.SweepElm;
...
public abstract class CircuitElm implements Editable {
    ...
    public void drawDots(Graphics g, Point pa, Point pb, double pos) {
        if (sim.mdc.stoppedCheck.getState() || pos == 0 || !sim.mdc.dotsCheckItem.isSelected()) {
            return;
        }
        int dx = pb.x - pa.x;
        int dy = pb.y - pa.y;
    ...
    public double updateDotCount(double cur, double cc) {
        if (sim.mdc.stoppedCheck.getState()) {
            return cc;
        }
    ...
    public void setVoltageColor(Graphics g, double volts) {
        ...
        if (!sim.mdc.voltsCheckItem.isSelected()) {
            if (!sim.mdc.powerCheckItem.isSelected()) // && !conductanceCheckItem.getState())
            {
                g.setColor(whiteColor);
            }
            return;
    ...
    public void setPowerColor(Graphics g, boolean yellow) {
        ...
        if (!sim.mdc.powerCheckItem.isSelected()) {
            return;
        }
